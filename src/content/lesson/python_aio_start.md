...
Кстати, при переходе на сайт происходит перенаправление, так и должно быть. Нажимаем на кнопку "Create" и для нас генерируется полноценный `.gitignore` файл на все случаи жизни. Файл сгенерирован, можно скопировать всё из браузера и вставить в новосозданный `.gitignore` в нашем проекте.
...
А что такое этот токен? Токен для бота, это как для обычного пользователя связка "логин-пароль" на любом сайте или в любом приложении. 
...
Это первый скрипт, бот просто запускается в терминале, и кажется как будто ничего не происходит. Если не было никаких ошибок - всё хорошо, бот запущен, он ждет сообщений от пользователей. Он пока не умеет обрабатывать никакие сообщения и команды, но он работает так или иначе. Если его остановить при помощи `Ctrl + C`, бот перестанет работать.
...
Декоратор в коде выше предоставляется самой библиотекой aiogram. 
Декоратор `@dp.message()` говорит о том, что асинхронная функция `echo_handler` будет обрабатывать все входящие сообщения адресованные нашему боту.

##### Типы данных, type hinting

Код `echo_handler(message: types.Message)` означает, что функция **echo_handler** будет принимать аргумент, у которого специальный тип `Message`. 
Этот тип не просто из самого python, как например `int` или `str`, но он определен в библиотеке aiogram. 
У него есть свои методы и свойства. Один из методов - `answer()`, он вызывается в функции `echo_handler`, и служит для того, чтоб отправить текстовый пользователю. 
А одно из свойств типа `message` - это `text`, которое из себя представляет текст входящего сообщения.

##### Обработчики команд

Команды для Telegram бота - простые текстовые сообщения боту, которые начинаются с `/`. Каждый бот имеет свой набор команд, но все боты должны уметь обрабатывать команду `/start`. Напишем обработчик команды `/start`:
...
Отличие декоратора в коде выше от декоратора функции echo_handler в том, что этот теперь отлавливает сообщение `/start`. 
Также в коде выше видно, что бот в этой функции будет обращаться к пользователю по его имени, которое можно взять из свойств объекта message. Вообще можно получить еще чуть больше информации об отправителе помимо его имени, например его уникальный ID или ник(username).


##### Подробный вывод свойств объекта в терминал

Иногда полезно посмотреть, какие свойства есть у объекта. Конечно подойдет обычный `print`. Но например результат такого кода  в терминале путает:
...
async def start(message: types.Message):
    ...
    await message.answer(f'Привет, {message.from_user.full_name}!')
```
Так как там очень много всего. Поэтому можно сделать вывод красивее:
...
@dp.message(Command('start'))
async def start(message: types.Message):
    # выводим всю информацию об отправителе
    pprint(vars(message.from_user))
    # или даже так, посмотрим на все свойства message, 
    # хотя не все они нам могут быть понятны и нужны
...
Вообще метод `answer()` - это легкая запись(синтактический сахар), для более общего способа отправлять сообщения при помощи экземпляра класса `Bot`:
